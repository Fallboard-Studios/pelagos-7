...
- [ ] **Task 1: `cabinetBreakpoints.ts` — viewport-width tiers + box heights**
  ...
  **Acceptance criteria:**
  - [ ] `CABINET_BREAKPOINT_MOBILE_MAX` (`640`) `< CABINET_BREAKPOINT_TABLET_MAX` (`1024`).
  - [ ] `CABINET_BOX_HEIGHT.mobile` (`32`) `< .tablet` (`40`) `< .desktop` (`48`).
  - [ ] `CabinetTier` is exported as `keyof typeof CABINET_BOX_HEIGHT`.
  - [ ] No other file is imported by this module — it has zero dependencies.

  **Verification:**
  - [ ] `npx vitest run src/utils/cabinetBreakpoints.test.ts` passes.
...
- [ ] **Task 2: `cabinetGeometry.ts` — oblique-projection wall math**
  ...
  **Acceptance criteria:**
  - [ ] `computeCabinetGeometry(w, h, 0)` returns `frontFaceOffsetX/Y === 0` and both wall polygons' points match the flat footprint (spec §1.2's `t=0` case).
  - [ ] `computeCabinetGeometry(w, h, 1)` returns `frontFaceOffsetX === 2·h`, `frontFaceOffsetY === h`, and both wall polygons' 3rd/4th points match that offset exactly.
  - [ ] `computeCabinetGeometry(w, h, 0.5)` is the exact midpoint of the `t=0`/`t=1` outputs (asserted against the formula, not just "doesn't throw").
  - [ ] Calling twice with identical arguments returns identical strings (pure function, no hidden state).

  **Verification:**
  - [ ] `npx vitest run src/utils/cabinetGeometry.test.ts` passes, covering every acceptance criterion above (spec §5's 4 listed cases).
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] **Task 3: `cabinetAnimation.ts` — pop/flat timing**
  ...
  **Acceptance criteria:**
  - [ ] `getCabinetPopDuration(true)` returns `0`.
  - [ ] `getCabinetPopDuration(false)` returns `CABINET_POP_DURATION`, which is `> 0`.
  - [ ] File shape mirrors `accordionAnimation.ts` (same export names pattern, same JSDoc convention referencing the prefers-reduced-motion precedent).

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/cabinetAnimation.test.ts` passes, mirroring `accordionAnimation.test.ts`'s own 2 cases.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] `grep -rn "cabinetBreakpoints\|cabinetGeometry\|cabinetAnimation" src/` shows all 3 new modules present with no consumer yet — no visual or behavioral change in the running app.
...
- [ ] **Task 4: `useCabinetBoxHeight` — live breakpoint-tier hook**
  ...
  **Acceptance criteria:**
  - [ ] Returns `48` (desktop) when neither the mobile nor tablet media query matches.
  - [ ] Returns `40` (tablet) when only the tablet query matches.
  - [ ] Returns `32` (mobile) when the mobile query matches, regardless of the tablet query's own state (mobile checked first).
  - [ ] Re-resolves when a stubbed query's `change` listener fires (not just on mount).
  - [ ] Cleans up both `change` listeners on unmount.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/useCabinetBoxHeight.test.ts` passes, using a `stubMatchMedia`-style helper generalized to answer the mobile and tablet queries independently (mirroring `AccordionContainer.test.tsx`'s own `stubMatchMedia` convention).
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] `npm run build:types`, `npm run lint`, `npm test` all clean.
- [ ] `useCabinetBoxHeight` is importable and correctly resolves all 3 tiers in isolation (verified by its own test suite) with zero other files in the app referencing it yet.
- [ ] Review with human before proceeding.
...
- [ ] **Task 5: `CabinetBox` — the shared cabinet-box rendering primitive**
  ...
  **Acceptance criteria:**
  - [ ] Renders `children` inside `.sc-cabinet-box__front`.
  - [ ] Registers a GSAP timeline via `setTimeline` when `popped` changes, once a non-zero width has been reported by the (mocked) `ResizeObserver`.
  - [ ] Calls `killTimeline` on unmount.
  - [ ] Still calls `setTimeline` under `prefers-reduced-motion` (stubbed true) — the timeline registers at `duration: 0`, it is not skipped outright (spec §3's explicit boundary).
  - [ ] Renders exactly one `.sc-cabinet-box__top-face` and one `.sc-cabinet-box__left-face` `<polygon>`, both inside an `aria-hidden`, `focusable="false"` `<svg>`.
  - [ ] `.sc-cabinet-box__walls` carries `pointer-events: none` in the CSS — never a hit-testable element.
  - [ ] No `foreignObject` anywhere in the file (spec §1.1).

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/CabinetBox.test.tsx` passes, using `vi.mock('@/animation/timelineMap', ...)`, the `stubMatchMedia` helper, and a `MockResizeObserver` (mirroring `useAutoSliderOrientation.test.ts`'s own class) — all 3 conventions this codebase already established, not new ones.
  - [ ] `npm run build:types`, `npm run lint` clean.
  - [ ] Manual check: none applicable yet — `CabinetBox` has zero real consumers until Task 6, same "component before consumer" precedent `DirectionalPanel`'s own Task 2 used.
...
- [ ] `CabinetBox` is importable and renders/animates correctly in isolation (verified by its own test suite) with zero other files in the app referencing it yet.
...
- [ ] **Task 6: `Button` — wired through `CabinetBox`**
  ...
  **Acceptance criteria:**
  - [ ] Every existing `Button.test.tsx` assertion (accessible name resolution, `onClick` firing, `disabled` blocking `onClick`) still passes unmodified.
  - [ ] `fireEvent.mouseEnter`/`mouseLeave` toggles the popped state.
  - [ ] `fireEvent.focus`/`blur` toggles the popped state independently of hover.
  - [ ] `fireEvent.pointerDown`/`pointerUp` toggles the popped state independently of hover/focus (spec §1.5 — the touch-support case).
  - [ ] A disabled button does not pop on `mouseEnter`/`focus`/`pointerDown` — the `!disabled` guard is directly tested, not inferred from platform behavior.
  - [ ] `ButtonSchema`/`ControlSchema` are untouched — `git diff src/types/controls.ts` is empty for this task.
  - [ ] `.sc-button`'s class name is unchanged (the `Console.css` cross-module dependency comment still holds).

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/Button.test.tsx` passes, covering every acceptance criterion above (spec §5's 4 new cases plus the existing 7).
  - [ ] `npm run build:types`, `npm run lint` clean.
  - [ ] `npm run build` clean.
  - [ ] Manual check (spec §5): open a drawer with a real `Button` (e.g. Robot Options' "Reset Melody"), confirm it renders flat at rest and pops fully on hover/focus/press at each of the 3 breakpoints (resize the viewport or use devtools device emulation past/under 640px and 1024px); confirm `Tab` focus shows the outline ring clearly on top of the popped box; confirm `CompanyCrudControls.tsx`'s Create/Delete Company buttons and `PingControlsDrawer.tsx`'s Reset Melody button never pop while their own `disabled` condition is true; confirm the pop snaps instantly with "reduce motion" enabled instead of animating.
...
- [ ] `npm run build:types`, `npm run lint`, `npm test`, `npm run build` all clean.
- [ ] Every real `Button` call site in the app (`HubNav`, `ConsolePanel`, `SectorSettingsDrawer`, `CompanyCrudControls`, `PingControlsDrawer`) renders through `CabinetBox` with no call-site changes required — confirmed by `npm run build:types` alone surfacing nothing, since the props contract didn't change.
- [ ] Manual pass (Task 6's own manual check) completed across at least one real disabled and one real enabled `Button` instance.
- [ ] Review with human before proceeding.
...
- [ ] **Task 7: `docs/CONSOLE_THEMING.md` — cabinet geometry/shading notes**
  ...
  **Acceptance criteria:**
  - [ ] `docs/CONSOLE_THEMING.md` documents the 2:1 oblique projection vector and the wall-polygon derivation, matching `cabinetGeometry.ts`'s actual shipped formula.
  - [ ] Documents the `color-mix()` face-shading technique and its relationship to `docs/specs/CONSOLE_THEMING.md § 1.3`'s own precedent.
  - [ ] No claim in the new section is contradicted by the actual shipped source.
...
- [ ] **Task 8: `docs/COMPONENT_LIBRARY.md` — Button's internal rendering note**
  ...
  **Acceptance criteria:**
  - [ ] `docs/COMPONENT_LIBRARY.md` documents that `Button` now renders through `CabinetBox` internally, with its props contract unchanged.
  - [ ] The note is spot-checked against `Button.tsx`'s actual shipped code (Task 6), not the spec's draft.

  **Verification:**
  - [ ] Manual review — spot-checked directly against the shipped `Button.tsx`.
