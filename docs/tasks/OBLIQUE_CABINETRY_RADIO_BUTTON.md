...
- [ ] **Task 1: `RadioButton` — one `CabinetBox` per option, selected-only accent tint, gapped/wrapping row**
  ...
  **Acceptance criteria:**
  - [ ] Every existing `RadioButton.test.tsx` assertion (8 cases: one item per option, exactly the matching option `aria-checked`, `onChange` on selection, no `onChange` on deselect-to-empty, accessible-name fallback to `schema.id`, not-disabled-by-default, every item `data-disabled` when `disabled`, no `onChange` when disabled) passes unmodified.
  - [ ] Renders exactly one `CabinetBox` per `schema.options` entry; `popped` is `true` only for the option whose `value` matches the component's own `value` prop, `false` for every other — computed in `RadioButton.tsx`, never read back from Radix's own `data-state`.
  - [ ] Each `CabinetBox` receives a distinct `timelineKey` of the form `` `cabinet-radio-${schema.id}-${option.value}` `` — no two options share a key, and the prefix is distinct from `cabinet-button-`/`cabinet-toggle-`/any `VoxelTrack` prefix.
  - [ ] No `CabinetBox` instance receives `boxHeight`, `frontWidth`/`frontHeight`, `popDistance`, `skipMountAnimation`, or `zIndex` — confirmed by direct inspection of the call site, not just by tests passing (an accidentally-passed default-matching value would still pass a naive test).
  - [ ] `option.label` renders as `CabinetBox`'s own `children`, not as `ToggleGroup.Item`'s direct text content.
  - [ ] `RadioButton.css`'s selected-only tint targets `.sc-radio-button__item[data-state='on'] .sc-cabinet-box__front` specifically — not an unconditional `.sc-radio-button__item .sc-cabinet-box__front` rule, which would tint every option regardless of selection.
  - [ ] `.sc-radio-button__root` no longer declares `border`, `border-radius`, or `overflow: hidden`; no `.sc-radio-button__item` rule declares `border-right`.
  - [ ] `useVoxelTrackGap` is imported from its existing location and called unmodified — no new breakpoint constant table, no rename of the hook or its exports.
  - [ ] `RadioButtonSchema`/`ControlSchema` (`src/types/controls.ts`) are untouched — `git diff src/types/controls.ts` is empty for this task.
  - [ ] `CabinetBox.tsx`/`.css`, `cabinetGeometry.ts`, `cabinetAnimation.ts`, `cabinetBreakpoints.ts`, `useCabinetBoxHeight.ts`, `VoxelTrack.tsx`, `voxelTrackMath.ts` are all untouched — `git diff` for each is empty for this task.
  - [ ] `disabled` continues to disable the whole `ToggleGroup.Root` at once; no per-item disabled logic is added.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/RadioButton.test.tsx` passes — the existing 8 cases plus the 4 new cases from spec §5 (per-option `popped`/`timelineKey` wiring via a mocked `CabinetBox`, label rendered as `CabinetBox`'s children, and a separate unmocked check that Radix's real `data-state` attribute still flips `'on'`/`'off'` with selection).
  - [ ] `npm run build:types` — zero TypeScript errors.
  - [ ] `npm run lint` — zero ESLint errors.
  - [ ] `npm run build` — production bundle builds cleanly.
  - [ ] `npm test` (full suite) passes, including `Lfo.test.tsx` (unmodified, composes `RadioButton` for its Shape row) and every drawer/domain-config test that renders a real `RadioButton` (Audio Setting, Decay Mode, per-layer Type, `CompanyButtonRow`'s own existing suite) without any call-site change.
  - [ ] Manual check (spec §5): across Audio Setting (4 short options), a Signature Array layer's Type row (4 short options), Decay Mode (2 options, no `loreLabel`), and the Companies drawer's button row (2–8 options) — confirm the selected option is the only one popped and accent-tinted, real spacing exists between boxes (no touching/shared-border segments), click/keyboard selection and the deselect-guard all still work, the focus ring renders on both states, no popped box's wall/glow visibly overlaps a neighbor at any breakpoint, and the Companies row specifically wraps to a second line (not scroll/clip) when it overflows, with a newly-created company's box using the ordinary pop-in flourish and a deleted company's box disappearing cleanly.

  **Dependencies:** None (11.1.1 and 11.1.3 already shipped and merged to `main`).

  **Files:** `src/components/ui/controls/RadioButton.tsx`, `src/components/ui/controls/RadioButton.css`, `src/components/ui/controls/RadioButton.test.tsx`

  **Estimated scope:** S (3 files, mechanical wiring against an already-proven `CabinetBox`/`useVoxelTrackGap`, same shape as 11.1.2's own `Toggle` consumer task)
...
- [ ] Every real `RadioButton` call site in the app (`AUDIO_SETTING_SCHEMA`, `DECAY_MODE_SCHEMA`, the per-layer Type schemas, `Lfo.tsx`'s Shape row, `buildCompanyButtonRowSchema`) renders through the new per-option `CabinetBox` with zero call-site changes required — confirmed by `npm run build:types` alone surfacing nothing, since `RadioButton`'s props contract didn't change.
- [ ] Manual check (Task 1's own verification list) complete against the real running app, across all four listed consumer shapes.
