...
**Status: all 14 tasks implemented and committed on `feature/header-hub-updates`. Every automated verification step is green. Manual (browser/real-layout) checks were not performed in this implementing session — no devtools tooling available — and remain open before this is considered fully done. Not yet reviewed with Crawford; not ready for PR.**
...
- [ ] **Manual checks still outstanding (no browser tooling in this implementing session):** Task 9's deadzone check, Task 11's click-through check, Checkpoint D's inline-merge/temperature-drift check.
...
| `MIN_VOLUME_RESERVE_PX` (Task 7, `80px`) and the exact volume-slider width in `Header.css` are unconfirmed engineering defaults | Low — cosmetic, easy to retune | Flagged in Task 7's own description; visual confirmation is part of the still-open manual checks |
...
2. `MIN_VOLUME_RESERVE_PX`'s exact value (`80px`) — a defensible default, not confirmed by eye against the real running app.
3. **New:** every manual/real-layout check across Tasks 9, 11, and the Final Checkpoint — genuinely blocking full "done" status, not a nice-to-have. No devtools/browser tooling was available in this implementing session (same limitation every prior Oblique Cabinetry task doc in this repo has recorded under the same circumstance).
