...
- [ ] **Task 1: `src/types/controls.ts` — `ControlSchema` discriminated union**
  ...
  **Acceptance criteria:**
  - [ ] All 13 variants exist with exactly the fields specified in spec §4 (bounds only where the grid specifies them: e.g. `RadioButtonSchema.options`, `SliderCenteredZeroSchema.min`/`max`).
  - [ ] `loreLabel`/`humanLabel` are optional on every variant (via `ControlSchemaBase`).
  - [ ] `LfoValue` extends the real `LfoSettings` from `src/types/lfo.ts` via `import type` — no engine import.
  - [ ] `CONTROL_SCHEMA_TYPES` has exactly 13 entries, no duplicates, matching the union's discriminants exactly.

  **Verification:**
  - [ ] `npx vitest run src/types/controls.test.ts` — asserts `CONTROL_SCHEMA_TYPES` length/uniqueness, and that one literal object per variant type-checks against `ControlSchema`.
...
- [ ] **Task 2: `DualLabel` component**
  ...
  **Acceptance criteria:**
  - [ ] Renders nothing (no wrapping `<div>`) when both `loreLabel` and `humanLabel` are omitted.
  - [ ] Renders only the lore span when only `loreLabel` is present, only the human span when only `humanLabel` is present, and both when both are present.
  - [ ] Zero hardcoded text — every rendered string comes from props.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/DualLabel.test.tsx` — covers all 3 presence cases (neither/one/both) explicitly.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] **Task 3: `Button` component**
  ...
  **Acceptance criteria:**
  - [ ] Renders its own schema's `loreLabel`/`humanLabel` via an internally-composed `<DualLabel />`.
  - [ ] Calls `onClick` exactly once per click; no internal state.
  - [ ] Accessible name resolves from `humanLabel` (falling back to `loreLabel`) via `aria-label`, when either is present.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/Button.test.tsx`.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] **Task 4: `Toggle` component**
  ...
  **Acceptance criteria:**
  - [ ] Renders `role="switch"` with `aria-checked` reflecting the controlled `value` prop — never manages its own selection state.
  - [ ] Calls `onChange(!value)` on click and on keyboard activation (Radix `Switch` provides the latter natively).
  - [ ] Renders its own schema's labels via internally-composed `<DualLabel />`.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/Toggle.test.tsx`.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] **Task 5: `TextInput` component**
  ...
  **Acceptance criteria:**
  - [ ] Renders `placeholder` from schema (not hardcoded) when present.
  - [ ] Respects `maxLength` from schema when present (native `maxLength` attribute).
  - [ ] Calls `onChange` with the raw string value on every keystroke; no internal buffering.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/TextInput.test.tsx`.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] `npm run build:types`, `npm run lint`, `npm test` clean.
- [ ] By-eye sanity check: nothing yet to compare against the robot editor (no numeric/bounded controls built yet) — skip this checkpoint's manual step, resume it from Phase 4 onward.
- [ ] Review with human before proceeding.
...
- [ ] **Task 6: `Stepper` component**
  ...
  **Acceptance criteria:**
  - [ ] Increment/decrement controls step by `schema.step ?? 1` and clamp at `min`/`max` — never call `onChange` with an out-of-bounds value.
  - [ ] Renders its own schema's labels via `<DualLabel />`.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/Stepper.test.tsx` — covers clamping at both bounds using the grid's own Density (1–16) and Motif Length (1–8) ranges as fixtures.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] **Task 7: `StepperWithToggle` component**
  ...
  **Acceptance criteria:**
  - [ ] Renders an actual `<Toggle />` and `<Stepper />` (imported, not reimplemented) — a test asserts both sub-components' interactive elements are present.
  - [ ] `Stepper`'s control is disabled when `value.active` is `false`, enabled when `true`.
  - [ ] `onChange` is called with the full `{ active, value }` shape on either the toggle or the stepper changing.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/StepperWithToggle.test.tsx`.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] **Task 8: `RadioButton` component**
  ...
  **Acceptance criteria:**
  - [ ] Renders one `ToggleGroup.Item` per `schema.options` entry, with exactly one marked pressed/selected matching `value`.
  - [ ] Calls `onChange(newValue)` on selection; a deselect-to-empty event (Radix's single-mode behavior) is guarded and does **not** call `onChange` — matching `RobotAudioTab.tsx`'s existing `if (value) handle...` guard.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/RadioButton.test.tsx` — uses the grid's LFO Shape options (TRIANGLE/SINE/SQUARE/SAWTOOTH) as a fixture.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] **Task 9: `SliderLinear` component**
  ...
  **Acceptance criteria:**
  - [ ] Matches spec §4's example exactly in shape (`schema`/`value`/`onChange` props, `DualLabel` composed internally).
  - [ ] Respects `min`/`max`/`step` from schema.
  - [ ] Renders `{value}{unit}` only when `schema.unit` is present.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/SliderLinear.test.tsx`.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] **Task 10: `SliderLog` component**
  ...
  **Acceptance criteria:**
  - [ ] `t = 0` maps to exactly `schema.min` (including the `min = 0` case) — not `LOG_EPSILON`.
  - [ ] The value→`t`→value round-trip is accurate within floating-point tolerance across the full range.
  - [ ] The midpoint (`t = 0.5`) is **not** the arithmetic mean of `min`/`max` (proves genuine log spacing, not linear) — asserted exactly, per the pattern `globalAudioSeed.test.ts` already uses for `scaleUnitValue`'s log/linear distinction, not statistically.
  - [ ] `onChange` receives the mapped display value, never the raw internal `t`.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/SliderLog.test.tsx` — uses the grid's Attack/Decay/Release bounds (0s–10s) as the `min = 0` fixture.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] **Task 11: `SliderCenteredZero` component**
  ...
  **Acceptance criteria:**
  - [ ] `value = 0` renders a zero-width (or negligible) fill.
  - [ ] Positive values fill from the zero point rightward; negative values fill from the zero point leftward.
  - [ ] The zero-point calculation is verified against an asymmetric-bounds fixture (e.g. `-20`/`+50`), not just the grid's symmetric `-50`/`+50`, to prove it isn't hardcoded to 50%.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/SliderCenteredZero.test.tsx`.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] By-eye check: `Stepper`, `SliderLinear`/`SliderLog`/`SliderCenteredZero` compared informally against `RobotAudioTab.tsx`'s still-live density/motif/octave/note-variance sliders — no code changes to that file.
- [ ] Review with human before proceeding.
...
- [ ] **Task 12: `CoordsInput` component**
  ...
  **Acceptance criteria:**
  - [ ] Renders two actual `<TextInput />` instances (not reimplemented raw `<input>`s).
  - [ ] Calls `onChange({ x, y })` with parsed numbers on either field changing.
  - [ ] A non-numeric entry does not throw and does not call `onChange` with `NaN`.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/CoordsInput.test.tsx`.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] **Task 13: `AccordionContainer` component**
  ...
  **Acceptance criteria:**
  - [ ] `npm install @radix-ui/react-accordion` completes; `package.json`/`package-lock.json` updated.
  - [ ] Toggles `aria-expanded` on its trigger when clicked.
  - [ ] Registers a GSAP timeline via `setTimeline` on expand/collapse and calls `killTimeline` on unmount (mocked in tests, matching `PowerRockerSwitch.test.tsx`'s mocking pattern).
  - [ ] Under `prefers-reduced-motion: reduce`, the section still opens/closes but the transition snaps instead of animating.
  - [ ] Renders its own schema's title via `<DualLabel />` in the trigger.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/AccordionContainer.test.tsx`.
  - [ ] `npm run build:types`, `npm run lint`, `npm run build` clean (confirms the new dependency resolves cleanly in the production bundle too).
...
- [ ] **Task 14: `Lfo` component**
  ...
  **Acceptance criteria:**
  - [ ] Renders actual `<RadioButton />`, `<SliderLinear />` (×2), and `<Toggle />` instances (not reimplemented).
  - [ ] `RadioButton`'s options match `LFO_SHAPES` from `src/types/lfo.ts` (triangle/sine/square/sawtooth) — imported, not re-listed as literals.
  - [ ] Rate/depth sliders' bounds match `LFO_RATE_MIN/MAX` and `LFO_DEPTH_MIN/MAX` from `src/types/lfo.ts` — imported, not re-hardcoded.
  - [ ] `onChange` receives a complete `LfoValue` on any sub-control changing.
  - [ ] No import of `src/engine/lfoEngine.ts` or any `Tone` object — type-only coupling, confirmed by this test file having zero engine mocks to set up.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/Lfo.test.tsx`.
  - [ ] `npm run build:types`, `npm run lint` clean.
...
- [ ] `npm run build:types`, `npm run lint`, `npm test`, `npm run build` all clean.
- [ ] All 13 primitives + `controls.ts` exist and are individually tested.
- [ ] Review with human before proceeding.
...
- [ ] **Task 15: `docs/COMPONENT_LIBRARY.md` + CLAUDE.md reference-list entry**
  ...
  **Acceptance criteria:**
  - [ ] Every one of the 13 primitives has a documented prop signature and its corresponding `ControlSchema` variant, cross-referenced against `ROBOT_DATA_GRID.md`'s rows.
  - [ ] The `DualLabel`-composition rule (every primitive renders its own schema's labels, no separate label schema) is stated explicitly, not left implicit.
  - [ ] Any new CSS custom property introduced during Tasks 2–14 is listed with its accent-tier classification; if none were introduced, the doc says so explicitly rather than omitting the section.
  - [ ] CLAUDE.md's reference doc list includes `docs/COMPONENT_LIBRARY.md`.

  **Verification:**
  - [ ] Manual review — every documented prop signature spot-checked against the actual shipped source, not reconstructed from memory (matching `LFO_INTEGRATION_PLAN.md` Task 15's own verification method).
  - [ ] `npm run build:types`, `npm run lint`, `npm test`, `npm run build` all clean (docs-only change, but confirms nothing else broke).
...
- [ ] `docs/COMPONENT_LIBRARY.md` reflects the shipped API — every signature spot-checked against source.
