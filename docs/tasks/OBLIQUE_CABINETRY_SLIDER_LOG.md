...
- [ ] **Task 3: `SliderLog` — rewired through `VoxelTrack`/`useVoxelTrackSlider`, box placement via `t`**
  ...
  **Acceptance criteria:**
  ...
  - [ ] `SliderLogSchema`/`ControlSchema` are untouched — `git diff src/types/controls.ts` is empty for this task.

  **Verification:**
  - [ ] `npx vitest run src/components/ui/controls/SliderLog.test.tsx` passes (17 unmodified + 3 rewritten + 3 new = 23 total).
  - [ ] `npm run build:types`, `npm run lint` clean.
  - [ ] `npm run build` clean.
  - [ ] Full suite (`npm test`) re-confirmed clean — including `SliderLinear.test.tsx` still unmodified and passing, and every other real `SliderLog` consumer's own test file (e.g. any `PingContourDrawer`-adjacent suite exercising Attack/Decay/Release), unmocked against the real component.
  - [ ] **Manual check:** confirmed via `ToolSearch` whether a browser-automation tool is available in this environment; if not (matching every prior Cabinetry item's own finding), flag as **outstanding for Crawford to perform in the running app** rather than skipping silently — load Ping Contour's Attack/Decay/Release at each of the 3 breakpoints, confirm the voxel-box track renders, confirm dragging/keyboard-stepping moves the straddling box non-linearly along the log curve (small drags near the low end travel further along the track than equal-sized drags near the high end), confirm the focus ring renders on top of the box row, confirm a narrow container clamps to 3 boxes and scrolls, confirm "reduce motion" snaps instead of animating, and spot-check that a real `SliderLinear` in the same drawer still looks/behaves unchanged.
...
- [ ] `npm run build:types`, `npm run lint`, `npm run build` all clean; `npm test` full suite passes.
- [ ] Every real `SliderLog` call site in the app renders through `VoxelTrack` with no call-site changes required — confirmed by `npm run build:types` alone surfacing nothing, since the props contract didn't change.
- [ ] Manual pass — explicitly flagged as outstanding if no browser-automation tool is available in this environment, matching how `OBLIQUE_CABINETRY_SLIDER_LINEAR.md`'s own Task 6 checkpoint recorded the same gap honestly rather than skipping it.
- [ ] Review with human before proceeding.
