...
* **Always:**
  ...
  * Keep every existing animation behavior: same two-step sequencing (height then fade on open, fade then height on close), same durations, same `prefers-reduced-motion` snap, same `timelineMap` registration and `killTimeline` cleanup.
...
* **Ask first (see §7):** unmounting on collapse; adding any new prop or public API to `AccordionContainer` (e.g. an `eagerMount`/`mountWhenClosed` opt-in); scheduling the mount of closed sections during idle time as a fallback for first-open hitching.
* **Never:**
...
4. **First-open cost is bounded:** a new harness step opens the single largest accordion on Fleet Params for the first time; record its longest task at 1× and 4×. Target **< 100 ms at 1×**; if it exceeds that, that is a finding recorded in the doc and raised (§7 Q5), not silently accepted.
...
### Open questions (each with a recommendation — please confirm or redirect)

* **Q1 — Keep mounted after first open, or unmount on collapse?** Recommended: **keep mounted** (§1.4: preserves `useLfoTargetGroup`'s local selection, avoids replaying mount cost, keeps the close animation simple). The alternative is a defensible memory trade-off but resets local state and needs a completion-callback unmount.
* **Q2 — Test strategy: shared `openAllAccordions` helper, or an `eagerMount` opt-in prop on the component?** Recommended: **the helper.** A prop adds public API and a production code path whose only user is tests; the helper keeps `AccordionContainer`'s contract exactly as it is.
* **Q3 — Scope: `AccordionContainer` only?** Recommended: **yes.** The 137 remaining Probes-list boxes are robot cards and the filter panel (not accordions), and Fleet Params' 20 are non-accordion chrome; those belong to 17.2.3 (per-box cost) and 17.2.5 (idle cost), not here.
* **Q4 — Are the §5.3 performance thresholds right?** They're proposals scaled from the box-count reduction, not measured predictions. Tighter (e.g. "no ≥100 ms task on any tile at 1×") may not be reachable for the Probes list without 17.2.3; looser risks shipping without a real audio improvement. Confirm or adjust before implementation so success isn't decided after the fact.
* **Q5 — If first-open of one section exceeds 100 ms at 1× (criterion 4), what then?** Recommended: record it and ship anyway if it's still a large net improvement over today's whole-tile stall (the mount cost is one section, user-initiated, not every tile switch) — but the fallback options are mounting closed sections during browser idle time after the tile opens, or a leaner per-box mount (17.2.3). Idle-time mounting is deliberately **not** in this spec's scope (it adds a scheduling mechanism and a state machine for a problem that may not materialize).

### Risks

* **R1 — Measuring unsettled content.** Today a section is measured after its content has long since settled (controls' `ResizeObserver`s and orientation/box-count hooks already ran while it sat closed). With lazy mounting, the first open measures content in its **first-commit** state; a control that resizes itself one frame later (e.g. a `useAutoSliderOrientation` 'auto' slider flipping axis, a `useVoxelTrackBoxCount` fit) could make the section slightly taller than the tween's target. Effect if it happens: `animateTo`'s `onComplete` sets `height: auto`, so it corrects at the end — potentially a small visible jump. Mitigation: the manual check in §5.3.7 at all three breakpoints; if seen, re-measure inside the tween's `onComplete` before releasing to `auto`. Most Audio Rig sliders declare an explicit orientation/`verticalHeight` (roadmap 13), which narrows the exposure.
