...
### 1.5 Self-fitting box count — a new hook mirroring `useAutoSliderOrientation`'s exact measurement convention

`useVoxelTrackBoxCount` observes the slider wrapper's *parent* via `ResizeObserver` — never the wrapper's own box, for the identical feedback-loop reason `useAutoSliderOrientation` already documents (rendering more boxes would widen the wrapper, which would widen a self-observed measurement, which would fit more boxes, unboundedly). It reads `width` or `height` from that observation depending on the *already-resolved* axis (never `'auto'` — orientation resolves first, per confirmed intent §1.6 below), and floors to the largest count that fits without overflowing, clamped to a minimum of 3 (`VOXEL_TRACK_MIN_BOX_COUNT`, confirmed intent):
