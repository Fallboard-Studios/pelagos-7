...
- **Phase 11.1 below still describes Cabinetry's face-shading as consuming this phase's *seed-driven* tokens — that premise no longer holds now that the tokens are static.** Not re-scoped here; needs revisiting whenever 11.1 is actually interviewed/specced. The tokens Cabinetry would consume (`--color-accent`/`--color-surface`) still exist and are still legible, they just don't change on retransmit anymore.
