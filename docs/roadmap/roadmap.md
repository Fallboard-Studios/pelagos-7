...
This phase populates our data engine with the complete global audio processing inventory and builds the corresponding Audio Rig drawer interface. We are populating src/data/audioRigConfig.ts with strongly typed ControlSchema definitions for all seven global effect blocks (Compressor, 3-Band EQ, Low-Pass Filter, High-Pass Filter, Chorus, Delay, and Reverb), specifying precise logarithmic bounds, center-zero offsets, units, default values, LFO modulation flags, and lore/human label pairings. Using these schemas, we are constructing the AudioRigDrawer layout, utilizing Accordion containers for each effect module and mapping over parameter schemas with our UI primitives. The entire drawer will render purely from data configs without hardcoded controls, leaving clean parameter IDs ready for Web Audio setter bindings in subsequent phases.
