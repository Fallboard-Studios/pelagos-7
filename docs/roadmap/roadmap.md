...
- **Face-shading helper** (`src/utils/cabinetShading.ts`, alongside `consoleTheme.ts`) deriving the Top/Left Face polygons' lighter/darker fill from the existing `--color-accent`/`--color-surface` tokens via `color-mix()`/HSL lightness offset, computed at render time — no new CSS custom properties.
- **`Button` cabinet behavior**, the foundation's first real consumer: a single box per control. Box width is variable, sized to the button's own content; height is 32px (mobile) / 40px (tablet) / 48px (desktop). Off state rests fully flat (0px offset, side walls collapsed); active/hover/focus/click pops fully out along the 2:1 vector, its magnitude scaling with box height (2×height right / 1×height down — e.g. +96px/+48px at the 48px desktop height; the other two breakpoints follow the same ratio). The clickable/touchable area extends to cover the full popped-out footprint too (CSS padding or an invisible `::before` matching the extrusion offset), so the hit target never shrinks to just the flat-state box.
...
- **`Toggle` cabinet behavior**: the same single-box pop/flat mechanism as `Button` (11.1.1) — same footprint math, same 2:1 vector scaled to box height, same extended hit area — but keyed off the control's `checked`/`active` value instead of a momentary click. Popped-out is the resting "on" state, not a transient click response; flat is "off."
...
Wires `SliderLinear` into the cabinet-box mechanism and, alongside it, builds the voxel-track rendering shared by all 3 sliders (`SliderLinear`, `SliderLog`, `SliderCenteredZero`) — a genuinely different rendering shape from Button/Toggle's single box, so it's built once here rather than three times. 11.1.4 and 11.1.5 each wire this same voxel-track system into their own primitive as thin follow-ups. Depends on 11.1.1 having shipped (reuses its cabinet-box primitive and face-shading helper); not yet interviewed/specced.

### Create

- **Voxel-track rendering**: a horizontal row of uniform cabinet boxes replacing the traditional track+handle — 32×32px facades 8px apart (mobile), 40×40px at 10px apart (tablet), 48×48px at 12px apart (desktop). A slider's rendered length becomes a fixed function of its box count (box size + gap, repeated), not a continuous track — sliders with multiple boxes now have effectively fixed widths/heights rather than filling arbitrary available space.
...
- **Extrusion-falloff**: the straddling box pops fully out (+100% of the box-height-scaled 2:1 vector 11.1.1 defines); boxes above it (toward the max, not yet filled) stay flat; boxes below it (toward the min, already filled) step down in equal decrements from the straddling box's 100% to 0% at the box nearest the minimum end, which does not extrude at all.
...
- Same `timelineMap` registration and Strict Separation constraint as 11.1.1/11.1.2, extended to the per-box extrusion/fill transitions — `onChange` still fires straight from Radix's `Slider.Root` drag/keyboard handling.

### About

The voxel-track mechanism built here is deliberately shared infrastructure, not `SliderLinear`-specific — 11.1.4 (`SliderLog`) and 11.1.5 (`SliderCenteredZero`) reuse it unchanged, swapping in only their own value-mapping math, so Volume, Detune, Gain, and every ADSR field still read as the same physical control family once all three ship. Exact per-slider box counts and where a given drawer's layout accommodates them is explicitly **not** this item's concern — box size, gap, and pop/fill behavior are fixed here; how many boxes fit in a given row is a later layout-rebuild phase's job (not yet numbered). Same layout/WorldView/robot-visual/Sleeve exclusions as 11.1.1.
