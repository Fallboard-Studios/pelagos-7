...
## 11. Console Theming & Cabinetry UI

Originally scoped as color tokens alone; folded together with a 2.5D "Oblique Cabinetry" rendering system for the interactive primitives once it became clear the two are load-bearing on each other — Cabinetry's face colors consume this phase's own seed-driven tokens directly, so there's no clean way to ship one without the other actually being usable.

### Create

- Pure seed-to-theme module (e.g. src/utils/consoleTheme.ts, alongside seedUtils.ts/getSeededVal.ts) computing bounded, legible HSL values — large/structural tokens (`--color-bg`, `--color-surface`) from the active AS seed; small/accent tokens (`--color-accent`, `--color-border`, button/text colors) from the active locale's coordinate seed
...
- **Oblique Cabinetry rendering system** for the Phase 1 Design System's interactive primitives (`Button`, `Toggle`, `SliderLinear`, `SliderLog`, `SliderCenteredZero`, `Stepper`, `StepperWithToggle`) — every cabinet visual is SVG, every transform/vertex morph is GSAP (~250ms, `power2.out`). **The primitive's existing Radix element (`Slider.Root`/`Track`/`Thumb`, `Switch`/`Toggle` root, or the native `<button>`) stays exactly as-is and keeps doing 100% of the real interaction** — pointer events, drag, keyboard, focus ring, ARIA state; the cabinet SVG is a `pointer-events: none` cosmetic layer stacked on top, reading that element's own `value`/`checked`/focus state as props to drive its pop/fill animation, never owning a hit-area itself. This is the same technique `SliderCenteredZero` already uses for its custom zero-anchored fill (Radix's own `Range` stays in the DOM, visually hidden, for structural/a11y parity — `docs/COMPONENT_LIBRARY.md`) — Cabinetry generalizes it to every primitive rather than inventing new hit-testing. That also *is* the "stationary hit box" the geometry needs: the front face slides out along a fixed 2:1 projection vector (2px right for every 1px down) as a pure visual, while the real interactive element underneath never moves, which is what actually prevents hover flutter (an element sliding away from the cursor and re-triggering its own hover state) — a side effect of keeping Radix in charge, not a separate mechanism to build. Opaque Top Face and Left Face `<polygon>`s anchor to the same fixed footprint and GSAP-morph their points to bridge the gap as the front face travels; labels/text live inside the front face's own `<g>` so they slide with it for free, no separate tracking.
- Runtime face-shading helper (e.g. `src/utils/cabinetShading.ts`, alongside `consoleTheme.ts`) deriving the Top/Left Face polygons' lighter/darker fill from the existing `--color-accent` (and inactive/surface) tokens via `color-mix()`/HSL lightness offset, computed at render time — no new CSS custom properties beyond what this phase's own token work already introduces.
- Button/Toggle cabinet behavior: a single box per control. Off state rests fully flat (0px offset, side walls collapsed); active/hover/focus/click pops fully out along the 2:1 vector (+48px right/+24px down at the 48px box size).
- Slider voxel-track rendering: a horizontal row of uniform 32×32px cabinet boxes spaced a quarter-box-width apart, replacing the traditional track+handle. Value readout is an opaque dual-fill on each box's front face — every box below the current value is 100% `--color-accent`, every box above is 100% inactive-color, and the one box straddling the exact float value is hard-split between the two at the local percentage. Extrusion follows the same value: the straddling box pops fully out (+100% vector), boxes above it toward the max stay flat, and boxes below it fall off toward flat — the single adjacent box at half-pop (+50%) on tracks under 5 total boxes, the two adjacent boxes at half-pop on tracks of 5 or more.
...
- `Button`/`Toggle`/`SliderLinear`/`SliderLog`/`SliderCenteredZero`/`Stepper`/`StepperWithToggle` (`src/components/ui/controls/`) re-render internally through the cabinetry system — same `schema`/`value`/`onChange` props contract as today (Phase 1's `ControlSchema` union is unchanged), so no domain config file (`audioRigConfig.ts`, `robotOptionsConfig.ts`, `sectorSettingsConfig.ts`, `companyConfig.ts`) needs to change and no call site needs to know its control went cabinet-shaped.
- Touch targets: Button/Toggle boxes render 48×48px wherever row space allows, 44×44px as the floor when it doesn't. Exact per-row box counts and slider lengths are explicitly **not** this phase's concern — box size and pop behavior are fixed here; how many boxes fit in a given row/slider is a later layout-rebuild phase's job (not yet numbered).
- Every cabinet pop/collapse and wall-polygon morph runs as a GSAP timeline registered in `timelineMap` (`setTimeline`/`killTimeline`), following `AccordionContainer`'s existing pattern (`docs/COMPONENT_LIBRARY.md`) — no timeline state in Zustand or React state, and (per CLAUDE.md's Strict Separation guardrail) the timeline only drives the cosmetic pop/morph, never calls `AudioEngine` itself — `onChange` still fires straight from Radix's own pointer/keyboard handler, same as every primitive today, since the underlying interactive element never changed.

### About

This phase gives the console itself a seed-derived visual identity, split by scale rather than by physical part: the AS seed drives large/structural areas — large background regions inside the Glass — while the active locale's coordinate seed drives small accent elements wherever they sit, buttons/text/borders on the Glass-side chrome. Colors stay bounded and legible the same way robot color generation already is (see ROBOT_DESIGN.md), since this is real interactive chrome sitting on a fixed dark background, not a decorative shape — a bad roll can't just look a little odd. Retransmitting a new seed in Sector Settings (Phase 5) visibly recolors the console, reinforcing the same "this is a piece of field equipment reporting what it's tuned to" fiction the rest of the console already leans on (SYSTEM_FIRMWARE_RESETS, the power-off confirm, etc.). Because the theme is a pure function of the seed and coordinates already being persisted by Sector Settings, it needs no separate storage of its own — Session Storage (Phase 12) restoring the seed automatically restores the look.

Layered on top of that token work, the Oblique Cabinetry system gives every interactive primitive a consistent 2.5D "physical switch" identity — flat and inert at rest, extruding toward the user when active — built entirely from the same seed-driven accent/surface tokens rather than a separate hardcoded palette, so a retransmit reshapes the cabinet faces' colors the same moment it reshapes everything else. The stationary-hit-box rule (front face slides, back frame never does, and only the back frame is ever listened to) is the load-bearing constraint that keeps hover state stable during the slide animation — this is a correctness rule, not a style preference. Slider voxels reuse the same dual-fill/extrusion-falloff logic regardless of which primitive or domain config renders them, so Volume, Detune, Gain, and every ADSR field all read as the same physical control family. Out of scope here: WorldView/terrain/sky styling (deferred to v2), robot visuals (locked to audio attributes per CLAUDE.md, untouched by this phase), the power rocker switch and the rest of the Sleeve casing (out of scope for now — see docs/CONSOLE_THEMING.md), and the actual per-row/per-slider box-count layout (deferred to a later layout-rebuild phase — this phase fixes box size and pop/fill behavior only).

### Docs

- No existing doc covers UI chrome theming — docs/CONSOLE_THEMING.md (in the design-doc style of SESSION_STORAGE.md, since this hasn't shipped yet either) documents the scale-based token split, the bounded-HSL generation rules, and — once this phase's scope folded in — the Oblique Cabinetry geometry/projection-vector/extrusion-state rules and the runtime face-shading approach. Already added to CLAUDE.md's reference doc list; its content needs a follow-up pass to cover the Cabinetry rules before this phase is actually implemented.
- docs/COMPONENT_LIBRARY.md gains a note once this phase ships: the affected primitives' internal rendering changed (cabinet SVG/GSAP instead of flat markup) while their `ControlSchema`/props contract stayed identical — worth calling out explicitly so it doesn't read as an undocumented behavior change.

## 11.1 Cabinetry Verification: Accessibility & Performance

Follow-up to 11, inserted the same way (out of sequence, not renumbering later phases), same as 10.1–10.4. Not a new feature — a dedicated verification pass once Cabinetry has actually shipped, before treating it as launch-ready.

### About

Cabinetry's design (11's own Restructure section) keeps each primitive's real Radix element in charge of all interaction and hands accessibility semantics off to Radix "for free," rather than rebuilding hit-testing/focus/ARIA from scratch — but that's an architectural intent, not a verified outcome, and this is a change touching every interactive primitive in the app at once. This phase is the check: a real keyboard-only walkthrough of every drawer (Robot Options, Audio Rig, Sector Settings, Company Manager), confirming the focus ring stays visible against a fully popped-out cabinet box (not obscured by the front face's z-index), confirming tab order wasn't disturbed by the SVG overlay's own DOM position, confirming `prefers-reduced-motion` actually cancels every cabinet timeline (pop, wall-polygon morph, and the voxel dual-fill transition alike) rather than just the ones the primitive spec called out first-hand, and a screen-reader pass over the slider voxel tracks specifically (the visual dual-fill/extrusion state carries real information — current value, how close to min/max — that must still be available to `aria-valuenow`/`aria-valuetext` via the underlying Radix `Slider`, not just implied visually).
