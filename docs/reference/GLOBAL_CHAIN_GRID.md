...
| Delay | `setGlobalDelay()` | delayTime | seconds, 0–1 | 0.05–0.5 | 0.25 | TEMPORAL REFLECTION MATRIX | PROPAGATION LAG | SLIDER | X |
...
- **Delay** `feedback` is capped at 0.95, short of Tone's technical 0–1, to avoid runaway buildup near unity. `delayTime`'s 0–1s full range now has an explicit `maxDelay: 1` set on the underlying `Tone.FeedbackDelay` node (previously relied on Tone's own default coincidentally matching) — the two must stay in sync if this range ever changes.
