...
- The module does not expose a public one-shot helper such as `scheduleAtBeat`; for one-shot timing, use the transport instance provided by AudioEngine or the engine-side scheduling path.
...
Trigger visual effects at measure boundaries. For one-shot events that need a precise beat position, use the transport instance provided by the engine layer rather than wall-clock timers.
