...
The trigger also carries a small round status light — decorative (`aria-hidden`; `aria-expanded` already carries the real state accessibly) — mirroring `PowerRockerSwitch`'s power light: red while closed, green while open (both keyed in CSS off the trigger's own Radix `data-state`, the same idiom `Toggle.css` uses for `data-state='checked'`), amber while the expand/collapse animation is in flight. That transitioning flag isn't React state — `animateTo()` sets/clears a `data-transitioning` attribute directly on the light via a ref, the same way it writes `el.style.height` directly, so the async GSAP `onComplete` callback never triggers a React update (avoids an `act()`-warning-prone state change firing from inside an animation callback).
