...
The trigger's contents, left to right: a decorative `+`/`−` open-state indicator, then the label — the label shifts right to make room for the indicator, via the trigger's own flex `gap` rather than a fixed offset. The indicator is `aria-hidden`; `aria-expanded` on the trigger itself already carries the real open/closed state accessibly, so it doesn't need to.
...
- **There is no status light.** An earlier revision had the trigger also render a `contentActive?: boolean`-driven light (green/red/unlit) reflecting a caller-supplied domain concept — e.g. `AudioRigDrawer` passing each effect block's own `enabled` state. Three separate passes removed every domain concept that used to drive it (per-effect Enabled, per-LFO Active, per-layer Active), leaving every call site's light unlit except one (Robot Drift's, still genuinely live — lit whenever that drift group had nonzero drift). The light was then removed entirely rather than kept for that one remaining case.
