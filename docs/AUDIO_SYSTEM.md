...
Key points:
- Each layer is an `OscillatorLayer` with `type` (a `WaveformType` — `'noise'` was removed in Roadmap Phase 9), `gain`, `detune`, `phase`, `active` (Baseline is always `true`; Coaxial/Harmonic are each independently toggleable), and optional `pulseWidth`. There is no per-layer `adsr` field — every layer shares the one envelope on `audioAttributes.adsr` (Roadmap Phase 9 collapsed Signature Array editing down to a single shared envelope per robot).
...
**User toggle — "Enable automatic effects".** `audioStore.audioSwellsEnabled` (default `true`) gates both pools' trigger/selection — `tickAudioSwells` checks it right before calling `maybeStartGlobalSwell`/`maybeStartRobotSwell`, skipping both when off. Advance always runs regardless of the toggle: a swell already in flight when the toggle flips off keeps ramping and returns to base on its own schedule, exactly as if nothing had changed — disabling only ever prevents *new* swells from starting, never cancels one in progress. `setAudioSwellsEnabled` is a plain state write with no `AudioEngine` call — unlike `setGlobalBypassEnabled`, there's no live node to touch; `audioSwells.ts` simply reads the flag fresh on its own next tick. The control lives in the Sector Settings drawer (`AUDIO_SWELLS_ENABLED_SCHEMA`, `data/sectorSettingsConfig.ts` — lore label "ENABLE DYNAMIC PING DEPLOYMENT", human label "Enable automatic effects"), deliberately outside the Attenuation Style/Coordinates `RetransmitInput` flow: it takes effect immediately on click and is never reset by a retransmit, seeded, or tied to any locale/Attenuation Style record.
