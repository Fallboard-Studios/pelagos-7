...
- **`connectLfoTarget`** resolves the live Signal via `AudioEngine.getRobotModulationTarget(robotId, target)` / `AudioEngine.getGlobalModulationTarget(target)`, then — critically — sets the LFO's `min`/`max` before calling `.connect()`. `Tone.LFO.connect()` sums onto the destination Param's existing value (native Web Audio behavior: connecting an input signal ADDS to whatever the Param's own intrinsic value already is — it never overrides it), so `min`/`max` are set to an **additive delta bounded by the target's CURRENT base value and its distance to the nearer edge of its own range** (`centeredSwingFromRange()`: `min(currentValue - rangeMin, rangeMax - currentValue)`, read off the resolved Signal's own `.value`), not the field's raw absolute range and not a fixed constant either. Two real, shipped bugs led here: (1) using the raw range directly — for a non-zero-centered field like LPF frequency (`20`–`20000`), that adds up to `+20000` Hz on top of whatever the slider is already at, pushing the actual cutoff past Nyquist — filter wide open, an audible burst of unfiltered harmonics — the instant the LFO connects; (2) a first fix used a *fixed* zero-centered swing (half the field's total span) instead — better, but for a base value sitting off-center (e.g. left low to dodge bug #1), a swing still that large pushed the combined value below the field's own minimum for roughly half of every cycle, heard as the mix muting for half the time. Bounding the swing by the base value's own position fixes both: `base ± swing` can never leave `[rangeMin, rangeMax]`, for any starting position. For a base value sitting exactly at the range's own midpoint (EQ dB bands, robot detune both default to `0`, the center of a symmetric range) this still yields the same "half the total span" swing — no behavior change there. Robot fields resolve from a small `ROBOT_LFO_FIELD_RANGE` table (gain `0–2`, detune `±50` cents, pulseWidth `0–1`, volume `0–1`); global targets reuse `GLOBAL_AUDIO_SEED_RANGES` (below), translating `lpf.`/`hpf.` target ids to that table's `filterLPF.`/`filterHPF.` keys — in both cases the *raw* range is what's resolved, then bounded around the current value before being applied.
...
- **`start` gates on the AudioContext, not the Transport.** A third real, shipped bug: `connectLfoTarget()` and `start()` are two separate calls (`audioStore`'s `setGlobalLfo` calls them back to back) — connecting always succeeds once a live Signal resolves, but `start()` used to no-op unless `Tone.getTransport().state === 'started'`. `AudioEngine.start()`'s own sequence makes the AudioContext running (via `Tone.start()`) well before the Transport itself finishes starting (instrument loading, waiting on reverb, then `Transport.start()`) — a real window where a user could toggle an LFO active fast enough to connect it to a live target while `start()` silently refused to actually start the oscillator. `Tone.LFO` outputs a raw, undepth-scaled "stopped" value for as long as it never starts — the waveform's value at its resting phase, not necessarily `0` (verified against `Oscillator.getInitialValue()`: `0` for a sine at phase `0`, but potentially a full `±1` for square/sawtooth/triangle or any non-zero phase) — which gets summed straight into whatever it's connected to, indefinitely, until the LFO is deactivated and reactivated once the Transport (by then) has caught up. Symptom: an audible step-change the instant the LFO connects, followed by the target parked at a wrong, static value — for a filter's frequency Param, potentially loud enough to sound like a burst of unfiltered harmonics, then silence if the resulting cutoff lands somewhere that blocks the mix. Gating on the AudioContext instead closes this window: the context becomes running essentially synchronously with the user's own gesture that triggers `AudioEngine.start()` in the first place, well before any of the slower instrument/reverb setup work.
...
- **Global-chain `LfoSettings`** (shape/rate/depth, plus whether each target starts active) are seed-generated per planet in `src/utils/globalAudioSeed.ts`'s `generateGlobalLfoSettings(planetId, planetName)`, sampling the same planet noise map `generateGlobalAudioSettings` uses, dataIds dot-namespaced as `globalLfo.<target>.<field>`. Unlike `GLOBAL_AUDIO_SEED_RANGES`, every target shares one global rate/depth range (`LFO_RATE_MIN/MAX`, `LFO_DEPTH_MIN/MAX`) rather than a per-field table, since `GLOBAL_CHAIN_GRID.md`'s `LFO?` column is a flat flag, not per-field bounds. `active` is seeded too — unlike the robot-level precedent above, where connected/active is purely a runtime UI concern never part of the generated data — using an `activeT >= 0.8` threshold (~20% chance per target, chosen so a typical planet seeds roughly 1–2 already-active LFOs out of 7, not several at once). Wired into `audioStore` via the same planet-sync subscription as `generateGlobalAudioSettings`, but deliberately **data-only** at that point (`regenerateGlobalLfoFromSeed` never calls `lfoEngine`) — planet-sync runs before any user gesture, and `lfoEngine`'s setters unconditionally construct a real `Tone.LFO` node on first use. `AudioEngine.start()` is what actually primes `lfoEngine` from this seeded state and connects+starts every already-active target, since that's the one point guaranteed to run after `Tone.start()` has succeeded.
