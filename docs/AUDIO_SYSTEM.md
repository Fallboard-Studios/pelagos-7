...
- **`connectLfoTarget`** resolves the live Signal via `AudioEngine.getRobotModulationTarget(robotId, target)` / `AudioEngine.getGlobalModulationTarget(target)`, then — critically — sets the LFO's `min`/`max` before calling `.connect()`. `Tone.LFO.connect()` sums onto the destination Param's existing value (native Web Audio behavior: connecting an input signal ADDS to whatever the Param's own intrinsic value already is — it never overrides it), so `min`/`max` are set to a **zero-centered swing at half the target's real span** (`centeredSwingFromRange()`), not the field's raw absolute range. Using the raw range directly was a real, shipped bug: for a non-zero-centered field like LPF frequency (`20`–`20000`), that adds up to `+20000` Hz on top of whatever the slider is already at, pushing the actual cutoff past Nyquist — filter wide open, an audible burst of unfiltered harmonics — the instant the LFO connects; and, its mirror image, requires pinning the base slider very low to avoid that crash, which then reads as near-total silence once the LFO disconnects and stops contributing anything on top. The zero-centered swing bounds the delta to the field's own span in either direction instead. For an already zero-centered field (EQ dB bands, robot detune) this is numerically identical to using the raw range — no behavior change there. Robot fields resolve from a small `ROBOT_LFO_FIELD_RANGE` table (gain `0–2`, detune `±50` cents, pulseWidth `0–1`, volume `0–1`); global targets reuse `GLOBAL_AUDIO_SEED_RANGES` (below), translating `lpf.`/`hpf.` target ids to that table's `filterLPF.`/`filterHPF.` keys — in both cases the *raw* range is what's resolved, then halved-and-centered before being applied.
