...
**The 4 drift groups** (`DriftGroupId`, `src/types/lfo.ts`): `'eq3' | 'filterLPF' | 'filterHPF' | 'robots'`. `driftGroupForTarget(target)` (`lfoEngine.ts`) classifies every target into exactly one, by prefix for the three global-chain groups (`eq3.*` → `eq3`, `lpf.*` → `filterLPF`, `hpf.*` → `filterHPF` — the only three global targets that ever carry an LFO at all) and falling through to `'robots'` for every `RobotLfoTargetId`, regardless of field or which robot — robot fields have no "effect block" concept to split by further.
...
**Seeding.** `GlobalAudioSettings.lfoDrift` (`types/globalAudio.ts`) is `Record<DriftGroupId, { rateDrift: number; depthDrift: number }>` — 4 independent pairs, both fields `-1..1`, default `0`. `generateGlobalAudioSettings` (`globalAudioSeed.ts`) samples all 8 values independently per planet via 8 dedicated `GlobalAudioSeedFieldKey` entries (`'lfoDrift.<group>.rateDrift'`/`'.depthDrift'` × 4 groups — a 3-level dotted path extending the same `effect.field` convention every other seeded field uses), each within a `-0.4..0.4` loading sub-window (`GLOBAL_AUDIO_LOADING_RANGES`) inside the full `-1..1` range — a first-pass default with no `GLOBAL_CHAIN_GRID.md` row to source from, since drift postdates that grid. `audioStore.ts`'s `applyGlobalAudioToEngine` pushes all 4 groups' values to `lfoEngine` in a loop over `DRIFT_GROUP_IDS`, reached the same two ways every other seeded field is: `regenerateGlobalAudioFromSeed` (planet switch) and `AudioEngine.start()`'s own re-apply-after-real-nodes-exist step.
