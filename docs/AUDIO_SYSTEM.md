...
**`globalBypass` fully silences the global pool.** `isGlobalTargetEligible` and `advanceGlobalSwell`'s own `stillEnabled` check both also gate on the Rig-wide Bypass (`globalAudio.globalBypass`) now, treated exactly like a per-effect `enabled: false` — a bypassed effect's targets are never eligible for a new swell, and an in-flight one is cancelled immediately and snapped to base (the same instant cancel-and-snap path a disabled effect already used, not the gradual `pingVarianceAutomation`-driven forced return above — Bypass is a hard kill switch, not an automation-amount gesture). Scoped to the global pool only; the robot pool has no relationship to the global effects chain and is completely unaffected.
