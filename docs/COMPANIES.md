...
`uiStore.ts` tracks `selectedCompanyId: string | null` and `allRobotsSelected: boolean`, independent of the existing `selectedRobotId` — selecting a robot never touches either. The two are mutually exclusive: `selectCompany` always clears `allRobotsSelected`, and `selectAllRobots` always clears `selectedCompanyId`. There is no third "nothing selected" state (the old "None"/"Reset" option was removed 2026-09-18) — the selection is always **All** (`allRobotsSelected: true`, the default) or one company. Selecting a company:
...
**"All"** (the default) activates the panel over every robot in the locale regardless of company (Freelance included) — added as a testing convenience (checking a broadcast edit, e.g. the Click Track toggle, across the whole locale used to mean selecting each company one at a time; see [BPM_CONTROL.md](tasks/BPM_CONTROL.md)'s "Post-launch addition: Company 'All' broadcast mode") and kept afterward as a general bulk-edit option. Unlike a specific company, "All" does **not** glow every robot in the world view — `isCompanyMember` is false for everyone, since a highlight on every robot would mark nothing apart. It has no `Company` object to bind to, so `CompanyCrudControls` (Rename/Delete) stays driven by `selectedCompanyId` alone — "All" only ever affects `CompanyOptionsSection`. The selection falls back to "All" when the selected company is deleted (`CompanyCrudControls`) and when a Retransmit regenerates companies with fresh ids (`retransmitWorld`); a selected company dropping to zero members instead leaves it selected, with the panel disabled (below).
...
1. **`CompanyButtonRow`** — "All" first, then one button per company (no Reset/None option). Reuses the `RadioButton` primitive (an options-list "one active among many" control RadioButton already implements, including active-state styling) rather than a bespoke button list.
2. **`CompanyCrudControls`** — Create (a locally-staged name draft pre-filled by a fresh `generateCompanyName` suggestion, fed by `Math.random()` rather than a seeded noise map since it's a live UI convenience roll, not replayable world generation; disabled at `MAX_COMPANIES`), Rename (a locally-staged name draft pre-filled with the selected company's current name, committed only by its own Submit button — disabled with none selected, a blank draft, or a draft unchanged from the current name), and Delete (disabled with none selected).
...
Reassignment happens through a `RadioButton` (`Select` through Roadmap Phase 10; replaced by Roadmap 10.5, [docs/specs/COMPANY_ASSIGNMENT_RADIO.md](specs/COMPANY_ASSIGNMENT_RADIO.md), once it turned out to be the Design System's only dropdown and its one real consumer — this one — didn't need a floating panel) shown in two places, both calling the same `localeStore.assignRobotToCompany(localeId, robotId, companyId | null)`:

- `RobotSelectionCard` (the list view)
- `RobotDisplaySection` (Robot Options, Phase 9)

The row lists every company by name plus "Freelance" (`FREELANCE_VALUE`, a non-empty sentinel kept for defensiveness/symmetry — see `docs/specs/COMPANY_ASSIGNMENT_RADIO.md` §1.2). `assignRobotToCompany` is one atomic store transition: it updates the robot's own `companyId` and both the old and new company's `robotIds` together, including the case where old and new are the same company (a genuine bug found in code review before merge — re-selecting the currently-assigned company must remove-then-re-add the robot from `robotIds`, not just remove it; see `localeStore.test.ts`'s regression coverage). Deleting a company (`removeCompany`) clears `companyId` on every member robot first — every robot in a deleted company becomes Freelance, mirroring `removeLocale`'s existing per-robot cleanup-before-removal pattern.

`RobotSelectionCard`'s company row still wraps it in a `stopPropagation` handler — the card itself is a clickable `role="button"` `<li>`, and a `RadioButton` option is a nested interactive DOM element that would otherwise bubble a click into the card's own activation handler. (Through Phase 10, when this was a `Select`, the same guard specifically had to be a `stopPropagation` handler rather than a DOM `closest()` check, since React re-propagates a Radix `Select` dropdown's portaled events along the *component* tree, not the DOM tree — a DOM-containment guard would have missed clicks on the dropdown's options, which rendered outside the card's DOM subtree via a portal. `RadioButton` has no portal at all, so that specific failure mode no longer applies, but the plain nested-interactive-element guard is still needed and is unchanged.)
