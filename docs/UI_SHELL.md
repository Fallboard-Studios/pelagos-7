...
- **`RobotsTab`'s list is a minimal stand-in**, not Phase 8's real Robot Selection card UI (avatar SVG, job title, audio status, battery status per card) — the Battery/Job data it needs now exists (Phase 7 shipped it), but the card UI itself is still Phase 8's job.
