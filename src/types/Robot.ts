...
export interface MelodyEvent {
   ...
   *
   * NOTE: this interface is structurally identical to but separately declared from
   * `RobotMelodyEvent` in `src/engine/melodyGenerator.ts` (pre-existing duplication, not
   * introduced by this field). Keep `pitchLocked` in sync on both — a value generated by
   * `generateMelodyForRobot` flows into `Robot.melody` via `updateRobot`, and reads of
   * `robot.melody[i].pitchLocked` (e.g. the docking re-roll) won't type-check against a stale copy.
   */
  pitchLocked?: boolean;
}
