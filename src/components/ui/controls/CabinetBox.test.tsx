...
describe('CabinetBox', () => {
  ...
  describe('fractional popped (0-1 number, not just boolean) — roadmap 11.1.3, VoxelTrack\'s extrusion-falloff', () => {
    it('computes geometry at t=0.4 directly for a fractional popped value', () => {
      render(<CabinetBox popped={0.4} timelineKey="test-box">x</CabinetBox>);
      const observer = MockResizeObserver.instances[0];
      act(() => observer.fire(100, 48));

      const tsUsed = (computeCabinetGeometry as ReturnType<typeof vi.fn>).mock.calls.map((args) => args[2]);
      expect(tsUsed).toContain(0.4);
    });

    it('tweens --cabinet-glow to exactly 0.4 (not 0 or 1) for a fractional popped value', () => {
      ...
      const glowCalls = fromToMock.mock.calls.filter(([target]) => target === wrapper);
      expect(glowCalls).toHaveLength(1);
      const [, , toVars] = glowCalls[0] as [unknown, Record<string, unknown>, Record<string, unknown>];
      expect(toVars['--cabinet-glow']).toBe(0.4);
    });

    it('animates from the previous fractional value directly on a real transition between two fractional values', () => {
      const { rerender } = render(<CabinetBox popped={0.4} timelineKey="test-box">x</CabinetBox>);
      const observer = MockResizeObserver.instances[0];
      act(() => observer.fire(100, 48));
      fromToMock.mockClear();
      ...
      const tsUsed = (computeCabinetGeometry as ReturnType<typeof vi.fn>).mock.calls.map((args) => args[2]);
      expect(tsUsed).toContain(0.4); // the "from" endpoint
      expect(tsUsed).toContain(0.7); // the "to" endpoint
      expect(tsUsed).not.toContain(0); // NOT the binary opposite of 0.7
      expect(tsUsed).not.toContain(1);
    });

    it('animates from the numeric opposite (1 - poppedT) on the very first render at a fractional value', () => {
      render(<CabinetBox popped={0.4} timelineKey="test-box">x</CabinetBox>);
      const observer = MockResizeObserver.instances[0];
      act(() => observer.fire(100, 48));

      const tsUsed = (computeCabinetGeometry as ReturnType<typeof vi.fn>).mock.calls.map((args) => args[2]);
      expect(tsUsed).toContain(0.6); // 1 - 0.4, the first-run "from" fallback
      expect(tsUsed).toContain(0.4); // the "to" endpoint
    });
  });
});
